...
class Fourier_skiplink(nn.Module):
    ...
    def __init__(self, dim, in_channel=1, project_channel=4, hidden_channel=8, pooling='max', mode=256,
                 ...
                 encoder_padding=1, outputActivate=True, device=None, middletruncation=False):
        super().__init__()
        self.device = None
        self.mode = mode
        self.dim = dim
        ...
        self.fold = (min((self.dim[0] // abs(self.mode)), 3)) if ((self.dim[0] // abs(self.mode)) > 0) else 1
        if (device is None):
        ...
        self.unet_Fencoder1 = self._Fencoder_convBlock(in_channel, project_channel, hidden_channel, encoder_ksize, encoder_padding,
                                                       ...
                                                       fold=self.fold,dim=self.dim, middletruncation=middletruncation).to(self.device)
        ...
        self.unet_Fencoder9 = self._Fencoder_convBlock(in_channel, project_channel, hidden_channel, encoder_ksize, encoder_padding,
                                                       ...
                                                       inverse=True, dim=self.dim, outputActivate=outputActivate).to(self.device)
    ...
    def forward(self, x):
        ...
        encoder = self.unet_Fencoder9(encoder)
    ...
    class _Fencoder_convBlock(nn.Module):
        ...
        def __init__(self, in_channel, out_channel, hidden_channel, ksize=(3, 3), padding=1 ,
                     ...
                     dim=None, skiplink=False, outputActivate=True, middletruncation=False):
            super().__init__()
            self.middletruncation = middletruncation
            self.mode=mode if (self.middletruncation is False) else abs(mode)
            self.fold = fold
            ...
            self.project = [
                                nn.Conv2d(out_channel*2 if (self.dim is None) else out_channel, in_channel,kernel_size=ksize,padding=padding),
                                self.activation
                            ]
        ...
        def forward(self, x, y=None):
            ...
            if (not self.inverse):
                ...
                if(self.mode <0):
                    ...
                    if (self.middletruncation):
                        midpoint = x.shape[3]//2
                        interval = self.mode//2
                        x = torch.concat([x[:, :, (self.mode * f):self.mode + (f * self.mode), (midpoint-interval):(midpoint+interval)] for f in range(self.fold)], dim=1)
                    else:
