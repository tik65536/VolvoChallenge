...
class Fourier(nn.Module):
    ...
    def __init__(self, dim, in_channel=1, project_channel=4, hidden_channel=8, pooling='max', mode=256,
        ...
        self.unet_Fencoder6 = self._Fencoder_convBlock(in_channel, project_channel, hidden_channel, encoder_ksize, encoder_padding,
                                                       hiddenactivation=self.hiddenactivation, activation=activation, mode=self.mode,
                                                       fold=self.fold, skiplink=False, outputActivate=fftActivate, concatlowhighFreq=concatlowhighFreq,
                                                       fftFilter_v=fftFilter_v, fftFilter_h=fftFilter_h, flipV=flipV).to(self.device)
        ...
        self.unet_Fencoder9 = self._Fencoder_convBlock(in_channel, project_channel, hidden_channel, encoder_ksize, encoder_padding,
                                                       ...
                                                       skiplink=False, inverse=True, dim=self.dim, outputActivate=outputActivate,
                                                       fftFilter_v=fftFilter_v, fftFilter_h=fftFilter_h, flipV=flipV).to(self.device)

    def forward(self, x):
        x = x.to(self.device)
        encoder, _ = self.unet_Fencoder1(x)
        encoder, _ = self.unet_Fencoder2(encoder)
        encoder, _ = self.unet_Fencoder3(encoder)
        encoder, _ = self.unet_Fencoder4(encoder)
        encoder, _ = self.unet_Fencoder5(encoder)
        encoder, _ = self.unet_Fencoder6(encoder)
        encoder, _ = self.unet_Fencoder7(encoder)
        encoder, _ = self.unet_Fencoder8(encoder)
    ...
    class _Fencoder_convBlock(nn.Module):
        ...
        def forward(self, x, y=None):
            ...
            if (not self.inverse):
                ...
                #x = torch.concat([x,y],dim=1) if (self.skiplink) else x
                x = self.project(x)
